...
PSgmlEdit tmp = this;
  do{
    ...
    if (tmp->name[0])
      Pos += swprintf(Dest+Pos,L"<%s",tmp->name);
    for (i = 0; i < tmp->parnum; i++){
      Pos += swprintf(Dest+Pos,L" %s=",tmp->params[i][0]);
      Pos += swprintf(Dest+Pos,L"\"%s\"",tmp->params[i][1]);
    }
    if (tmp->name[0])
      Pos += swprintf(Dest+Pos,L">\r\n");
    if (tmp->gettype() == EPLAINEL)
      Pos += swprintf(Dest+Pos,L"%s\r\n", tmp->content);
    if (tmp->echild)
    ...
    if (tmp->gettype() == EBLOCKEDEL){
      for(i = 0; i < Lev*SP; i++)
        Pos += swprintf(Dest+Pos,L" ");
      Pos += swprintf(Dest+Pos,L"</");
      if (tmp->name) Pos += swprintf(Dest+Pos,L"%s",tmp->name);
      Pos += swprintf(Dest+Pos,L">\r\n");
    };
    tmp = (PSgmlEdit)tmp->enext;
  }while(tmp);
